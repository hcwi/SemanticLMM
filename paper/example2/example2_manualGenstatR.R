...
example2_genstat <- function() {
  ...
  dataset <- Dataset(label="Dataset_example2",
                     ...
                     comments = list("rdf:value \"
Treatment,Block,y1,y2,y3,y4;
T1,B1,6,2,6,4;
T1,B2,4,3,4,6;
T2,B1,5,5,7,2;
T2,B2,6,1,3,4;
...
T3,B2,3,8,7,9;\""))
  ...
  var3 <- CategoricalVariable(label = "Block", 
                              levels = list("B1", "B2"), 
                              type="IndependentVariable")
  #var4 <- ContinuousVariable(label = "Y", type="DependentVariable")
  ...
  for (i in 1:length(var1$levels)) {
    ...
    for (j in 1:length(var2$levels)) {
      ...
      for (col in 1:row) {
        ...
        #lab <- paste(t,lab1,lab2,sep="_")
        #effect <- Parameter(label=lab, levels=list(eff1est,eff2est), type = list(t))
        
        # estimates
        #est <- Estimate(lab, value = cov[row,col], parameter = effect, type=list("Estimate"))
        #est$se <- se[row,col]
        #effect$estimate <- append(effect$estimate, est)
